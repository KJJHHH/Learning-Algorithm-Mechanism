...
2. Too much epoch could shows overfitting but since the full learning algorithm has the mechanism to cope with overffiting, reorganising, so for the weight tuning module after selecting LTS could set to higher epochs. With the epoch in weight tuning and regularising modules in reor
