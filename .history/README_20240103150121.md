...
3. Process | Full Path
    ...
    - Benchmark: simple fully connected net (2 ~ 3)\
        Trainloss / Testloss/ BestEpochin10000epochs(BasedonTestloss) / Traintime

        |          | Two layer net      | Three layer net        | Four layer net |
        | -------- | ------------------ | ---------------------- | -------------- |
        | Copper   | 0.93/14.17/2314/4  |  37.60/1138.72/        |                |
...
1. The Full module epochs too small, need to add to around 2000\
    Improvement: Set the epochs and learning goals based on fully connected result (learning goal is the train data max residual of full learning algorithm should be less than maximum residual of the best epoch in fully connected nets)
2. Too much epoch could shows overfitting but since the full learning algorithm has the mechanism to cope with overffiting, reorganising, so for the weight tuning module after selecting LTS could set to higher epochs. With the epoch in weight tuning and regularising modules in reorganising module, to 
