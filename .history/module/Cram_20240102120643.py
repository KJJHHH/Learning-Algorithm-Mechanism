...
class cramming(nn.Module):
    def __init__(self, model, X_train, y_train, lr_goal, s):
        ...
        self.s = s
        self.ks_mask = \
            abs(self.model(self.X_train).reshape(-1) - self.y_train) > self.lr_goal
    

    def cram(self):
        ks = torch.nonzero(self.ks_mask, as_tuple=False)
        for i, k in enumerate(ks):
            print(f"cramming sample {k.item()}th |{i/len(ks)*100:.2f}% total of {len(ks)}")
    ...
    def cram_add_node(self, r, k):
        ...
        for name in param:
            if name == 'layer_1.weight':
                ...
                new_model.layer_1.weight = new_w
                    
            if name == 'layer_1.bias':
                # First node
                node_add = self.s - torch.dot(r, self.X_train[k].reshape(-1))
                new_b = torch.cat([param[name], node_add.reshape(1)], dim = 0)
                # Second node
                node_add = (-1) * torch.dot(r, self.X_train[k].reshape(-1))
                new_b = torch.cat([new_b, node_add.reshape(1)], dim = 0)
                # Third node
                node_add = (-1)*self.s - torch.dot(r, self.X_train[k].reshape(-1))
                new_b = torch.cat([new_b, node_add.reshape(1)], dim = 0)

                new_model.layer_1.bias = new_b
            ...
            if name == 'layer_out.weight':
                ...
                new_model.layer_out.weight = new_w_o
