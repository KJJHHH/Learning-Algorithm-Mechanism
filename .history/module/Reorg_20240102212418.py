...
class reorganising():
    def __init__(
        ...
        if os.path.exists(self.acceptable_path):
            self.out_file(f"Acceptable SLFN exist in '{self.acceptable_path}'.")
            self.model = torch.load(self.acceptable_path)
        else:
            self.out_file(f"Acceptable SLFN not exist in '{self.acceptable_path}'.")
            self.model = None        
    ...
    def module_weight_EU_LG_UA(self, model):
        ...
        for epoch in range(self.epochs):
            ...
            if self.print_w_tune:
                self.out_file(f"max eps sqaure: {max(eps)}")
            ...
            if train_loss <= loss_old:
                ...
                if optimizer.param_groups[0]['lr'] < self.lr_bound:
                    self.out_file(f"non acceptable module at max eps {max(eps)}")
        ...
        while k<=self.hidden_dim:
            ...
            if acceptable == True and train_loss_list[-1] < loss_reg:
                ...
                if self.hidden_dim == 1:
                    pself.out_fileint("model only 1 hidden nodes")
