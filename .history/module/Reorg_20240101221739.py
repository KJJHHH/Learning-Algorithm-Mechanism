...
class reorganising():
    def __init__(
            self, previous_module, train_loader, test_loader, hidden_dim,
            criterion = nn.MSELoss(), 
            epochs = 50, 
            lr_reg = .001, 
            lr_w_tune = .001, 
            lr_bound_reg = .00001, 
            lr_bound_w_tune = .00001, 
            eps_reg = .0001, 
            eps_w_tune = .0001,
            print_reg = False,
            print_w_tune = False,
        ...
        self.lr_reg = lr_reg
        self.lr_w_tune = lr_w_tune

        self.lr_bound_reg = lr_bound_reg
        self.lr_bound_w_tune = lr_bound_w_tune
        self.eps_reg = eps_reg
        self.eps_w_tune = eps_w_tune
    ...
    """
    def module_weight_EU_LG_UA(self, model, optimizer):
        """
        ...
        for epoch in range(self.epochs):
            ...
            # stopping criteria 1
            acceptable, max_eps, y_pred = check_acceptable(self.train_loader, model, self.eps_w_tune)

            if self.print_w_tune:
                print(f"max eps sqaure: {max_eps}")
            if acceptable:
                print(f"acceptable module at max eps {max_eps}")
            ...
            if train_loss <= loss_old:
                ...
                if optimizer.param_groups[0]['lr'] < self.lr_bound_w_tune:
    ...
    def regularising_EU_LG_UA(self):  
        ...
        for epoch in range(self.epochs):
            ...
            # stopping criteria 1
            acceptable, max_eps, y_pred = check_acceptable(self.train_loader, self.model, self.eps_reg)
            if self.print_reg:
                print(f"max eps sqaure: {max_eps}")
            if max_eps < self.eps_reg:
                print(f"acceptable module at max eps {max_eps}")
                return acceptable, train_loss_list, test_loss_list
            ...
            if train_loss <= loss_old:
                ...
                if optimizer.param_groups[0]['lr'] < self.lr_bound_reg:
                    acceptable = False
                    print(f"Lr too small, non acceptable module at max eps {max_epseps}")
                    return acceptable, train_loss_list, test_loss_list            
                else:
                    if self.print_reg:
                        print("Restore model and lr decrease")
    ...
    def reorganising(self):
        ...
        while k<=self.hidden_dim:
            ...
            optimizer_w_tuning = optim.Adam(trim_model.parameters(), lr = self.lr_w_tune)
            ...
            acceptable, model, train_loss_list, test_loss_list = \
                self.module_weight_EU_LG_UA(trim_model, optimizer_w_tuning)
