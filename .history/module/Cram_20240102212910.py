...
class cramming(nn.Module):
    def __init__(self, model, X_train, y_train, lr_goal, s, out_file, dtype = torch.float64):
    ...
    def cram(self):
        ...
        for i, k in enumerate(ks):
            self.out_file.write(f"cramming sample {k.item()}th |{i/len(ks)*100:.2f}% total of {len(ks)}")
            self.cram_find_r(k)
    ...
    def cram_find_r(self, k): 
        ...
        if torch.any(torch.all(X_no_k == self.X_train[k], dim=1)):
            self.out_file.write("If X_train[k] in X_train: yes. check again")
        
        n = 0
        while True:
            n+=1
            self.out_file.write(f"try vector {n}", end="\r")

            r = torch.rand(self.input_dim, dtype = self.dtype)          
