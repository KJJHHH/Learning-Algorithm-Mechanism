...
class reorganising():
    ...
    def module_weight_EU_LG_UA(self, model):
        ...
        for epoch in range(self.epochs):
            ...
            if self.print_w_tune:
                self.out_file.write("train_loss:", train_loss)
            ...
            if self.print_w_tune:
                self.out_file.write(f"max eps sqaure: {max(eps)}")
            
            # adjust lr
            if train_loss <= loss_old:
                if self.print_w_tune:
                    self.out_file.write("Save model and lr increase")
                ...
                if optimizer.param_groups[0]['lr'] < self.lr_bound:
                    self.out_file.write(f"non acceptable module at max eps {max(eps)}")
                    return acceptable, model, train_loss_list, test_loss_list            
                else:
                    if self.print_w_tune:
                        self.out_file.write("Restore model and lr decrease")
    ...
    def regularising_EU_LG_UA(self):  
        ...
        for epoch in range(self.epochs):
            gc.collect()
            self.out_file(f"|{epoch/self.epochs*100: .2f}%| Epoch {epoch}", end="\r")
            ...
            # print
            if self.print_reg:
                self.out_file("train_loss:", train_loss)
                self.out_file(f"regular term: {self.reg(self.hidden_dim, X.shape[0])}")
            ...
            if train_loss <= loss_old:
                if self.print_reg:
                    self.out_file("Save model and lr increase")
                ...
                if optimizer.param_groups[0]['lr'] < self.lr_bound:
                    ...
                    if self.print_reg:
                        self.out_file("Restore model and lr decrease")
                    optimizer.param_groups[0]["lr"] *= 0.8
    ...
    def trim_model_nodes(self, k):
        ...
        if False not in delete_status:
            self.out_file("Try trim model: Copy model and delete nodes success")
        else:
            self.out_file("Try trim model: Copy model and delete nodes error")
    ...
    def reorganising(self):
        ...
        self.out_file('=================== reorganising ===================')
        k = 1 
        check_n = 0
        while k<=self.hidden_dim:
            self.out_file(f"[{check_n/self.hidden_dim*100: .2f}%] ------------> Checking nodes...")
            self.out_file(self.model)            
            self.out_file(f"    --> Start regularising_EU_LG_UA")
            check_n += 1
            ...
            self.out_file(f"    --> Start module_EU_LG")
            ...
            if acceptable == True and train_loss_list[-1] < loss_reg:
                self.out_file("!!!Trim node from the model, hidden nodes decrease by 1!!!")
                self.hidden_dim-=1
                self.model = trim_model
                if self.hidden_dim == 1:
                    self.out_file("model only 1 hidden nodes")
