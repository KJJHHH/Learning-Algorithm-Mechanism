...
class reorganising():
    def __init__(
            self, train_loader, test_loader, hidden_dim,
        ...
        self.acceptable_wt_path = f'acceptable/{previous}.pth'
    ...
    def regularising_EU_LG_UA(self):  
        ...
        for epoch in range(self.epochs):
            ...
            for _, (X, y) in enumerate(self.train_loader):
                y = y.reshape(-1, 1)
                preds = self.model(X)
                eps = torch.cat([eps, (y-preds)], axis = 0)
            if self.print_reg:
                print(f"max eps sqaure: {max(eps)}")
    ...
    def trim_model_nodes(self, k):
        ...
        """
        trim_model = TwoLayerNet(9, self.hidden_dim-1, 1)
