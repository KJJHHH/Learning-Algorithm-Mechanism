...
class cramming(nn.Module):
    def __init__(self, model, train_loader, X_train, y_train, indices_to_cram, lr_goal, s):
        ...
        self.eps,
        self.y_pred
        ) = check_acceptable(train_loader, self.model, self.lr_goal)
        self.s = s
        self.ks = torch.where(self.eps > self.lr_goal)[0]
