...
class reorganising():
    def __init__(
        ...
        if os.path.exists(self.acceptable_path):
            write(self.out_file, f"Acceptable SLFN exist in '{self.acceptable_path}'.\n", False)
            self.model = torch.load(self.acceptable_path)
        else:
            write(f"Acceptable SLFN not exist in '{self.acceptable_path}'.\n", False)
            self.model = "no acceptable "       
    ...
    def reorganising(self):
        ...
        while k<=self.hidden_dim:
            write(self.out_file, f"---> Checking nodes...{check_n}/{self.hidden_dim*100: .2f} \n", False)
            write(self.out_file, f"{str(self.model)}\n", False)            
            write(self.out_file, f"    --> Start regularising_EU_LG_UA\n", False)
            check_n += 1
            ...
            if acceptable == True and train_loss_list[-1] < loss_reg:
                ...
                if self.hidden_dim == 1:
                    write(self.out_file, "model only 1 hidden nodes\n", False)
