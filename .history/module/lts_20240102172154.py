...
def lts(model, X_train, y_train, lr_goal):
    ...
    eps = torch.square(y_train_lts.to(device) - model(X_train_lts.to(device)))
    print(eps[-10:])
    acceptable, eps, y_pred = check_acceptable(train_loader, model, lr_goal, X_train_lts, y_train_lts)
    print(eps)
